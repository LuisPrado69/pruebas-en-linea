...
?>
  ...
  <script>
      function opciones(val)
      {
        ...
        if(opcion==1)
        {
          window.location= "modificar_respuestas.php?id="+val;
        }
  ...
  <section class="content">
    ...
    echo '<div class="alert ' . $statusMsgClass . '">' . $statusMsg . '</div>';
        ...
        <div class="panel-heading">
            Lista de Respuestas
            <a href="javascript:void(0);" onclick="$('#importFrm').slideToggle();">Subir Respuestas</a>
        </div>
        <span class="section">
              <?php
            ...
            <table class="table table-bordered table-striped" id="example1" style="text-align: center">
                ...
                <tbody>
                    <?php
                    //get rows query
                    $query = $db->query("SELECT * FROM answer ORDER BY qid aSC");
                    if ($query->num_rows > 0) {
