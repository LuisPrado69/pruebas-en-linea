...
?>
  ...
  <script>
      function opciones(val)
      {
        ...
        if(opcion==1)
        {
          window.location= "modificar_ayuda.php?id="+val;
        }
...
<div class="content-wrapper">
  ...
  <section class="content">
        ...
        <h2 id="otro"align="center"><font color="#36A4C4">Listado de Ayudas</font></h2>
    <?php if (!empty($statusMsg)) {
    echo '<div class="alert ' . $statusMsgClass . '">' . $statusMsg . '</div>';
}?>
        <div class="panel-heading">
            Lista de Ayudas
            <a href="javascript:void(0);" onclick="$('#importFrm').slideToggle();">Subir Ayudas</a>
        </div>
        <span class="section">
              <?php
...
?>
        ...
        <div class="panel-body">
            <form action="SubAyu.php" method="post" enctype="multipart/form-data" id="importFrm">
            ...
            <table class="table table-bordered table-striped" id="example1" style="text-align: center">
                ...
                <tbody>
                    <?php
                      //get rows query
                      $query = $db->query("SELECT * FROM ayuda ORDER BY qid aSC");
                      if ($query->num_rows > 0) {
                          while ($row = $query->fetch_assoc()) {
                              ?>
                    <tr>
                      <td>Pregunta: <?php echo $row['qid']; ?></td>
                      <td><?php echo $row['ayuda']; ?></td>
                      <td>
